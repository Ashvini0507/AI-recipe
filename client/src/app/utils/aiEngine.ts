...
export function getRecommendedRecipes(
  ...
  mealFilter: 'all' | 'breakfast' | 'lunch' | 'dinner' | 'snack' = 'all'
): RecipeScore[] {
  // Filter recipes based on type
  let filteredRecipes = filterType === 'all' 
    ? allRecipes 
    : allRecipes.filter(recipe => recipe.type === filterType);

  if (mealFilter !== 'all') {
    filteredRecipes = filteredRecipes.filter(recipe => 
      !recipe.mealType || recipe.mealType.includes(mealFilter)
    );
  }
...
export function searchRecipesByName(
  ...
  return recipes.filter(recipe => {
    // original english title match
    const baseMatch = recipe.title.toLowerCase().includes(query);
    // translated title (may be same as base) also match
    const translated = i18n
      .t(`recipes.${recipe.id}.title`, { defaultValue: recipe.title })
      .toLowerCase();
    const translationMatch = translated.includes(query);
    return baseMatch || translationMatch;
  });
}
